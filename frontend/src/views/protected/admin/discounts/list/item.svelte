<script>
  ...
  function discountFigure() {
    return discount.percentage > 0 ? `${discount.percentage}%` : discount.amount;
  }
</script>

<tr class="even:bg-gray-100 text-gray-700">
  <td class="py-3 pl-4">
    <a class="text-link" href={`/admin/discounts/show/${discount.id}`}>{discount.name}</a>
  </td>
  <td class="py-3 text-right">{discountFigure()}</td>
  <td class="py-3 text-right">{formatDate(discount.startAt)}</td>
  <td class="py-3 text-right">{discount.endAt ? formatDate(discount.endAt) : 'No end date'}</td>
  <td class="py-3 text-right">{discount.active}</td>
  <td class="py-3 pr-4 text-right flex justify-evenly">
    <svg viewBox="0 0 20 20" fill="currentColor" class="pencil w-6 h-6 text-blue-500">
      <path
        d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3
        14.172V17h2.828l8.38-8.379-2.83-2.828z" />
    </svg>
    <a href="#!" class="ml-2" on:click={() => dispatch('removeDiscount', discount.id)}>
