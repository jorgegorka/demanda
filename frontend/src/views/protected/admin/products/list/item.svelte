...
<tr class="even:bg-gray-100 text-gray-700">
  <td class="py-3 pl-4">
    <a class="text-link" href={`/admin/products/show/${product.id}`}>{product.name}</a>
  </td>
  <td class="py-3 text-right">{product.category ? product.category.name : ''}</td>
  ...
  <td class="py-3 pr-4 text-right flex justify-evenly">
    ...
    <a href="#!" class="ml-2" on:click={() => dispatch('removeProduct', product.id)}>
